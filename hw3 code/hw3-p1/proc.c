...
void signal_deliver(int signum)
{
	...
	*((uint*) (proc->tf->esp - 16)) = proc->tf->edx;			// edx
	//hw3-p1
	//pushing sig_info
	*((uint*) (proc->tf->esp - 20)) = (uint) proc->sig_info.type; //sig_info type
	*((uint*) (proc->tf->esp - 24)) = (uint) proc->sig_info.addr; //sig_info addr
	*((uint*) (proc->tf->esp - 28)) = (uint) signum; //signal num
